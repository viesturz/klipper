...
class McuAnalogPin(override val mcu: Mcu, val config: config.AnalogInPin, configure: McuConfigure): McuComponent,
    ...
    private val nextClockOffset: Double = config.reportInterval - config.sampleTime.toDouble() * config.sampleCount.toDouble()
    private val adcInverse = 1.0f/configure.identify.adcMax
    private var _value = config.minValue
    override val value: Float
        get() = _value

    override fun configure(configure: McuConfigure) {
        if(nextClockOffset < 0.002) throw ConfigurationException("Analog pin ${config.pin} report interval too close.")
        ...
        { clock ->
            addId(id); addU(clock); addU(configure.durationToTicks(config.sampleTime)); addU(config.sampleCount);
            addU(configure.durationToTicks(config.reportInterval));
            addHU((config.minValue * configure.identify.adcMax).toUInt().toUShort());
            addHU((config.maxValue * configure.identify.adcMax).toUInt().toUShort());
    ...
    override fun setListener(handler: suspend (p: AnalogInPin.Measurement) -> Unit?) {
    ...
    fun handleAnalogInState(response: ResponseAnalogInState) {
        ...
        _value = response.value.toFloat() * adcInverse
        logger.info { "value: $_value" }
        listener?.let { l ->
            runtime.reactor.runNow { l(AnalogInPin.Measurement(time, _value))} }
    }
}

data class ResponseAnalogInState(override val id: ObjectId, val nextClock :UInt, val value: UShort): McuObjectResponse
